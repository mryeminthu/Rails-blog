...
RSpec.describe Post, type: :model do
  ...
  describe '#five_most_recent_comments' do
    it 'returns 5 most recent comments' do
      ...
      comment_one = Comment.create(user: user, post: post, text: 'comment 1', created_at: 10.days.ago)
      comment_two = Comment.create(user: user, post: post, text: 'comment 2', created_at: 9.days.ago)
      comment_three = Comment.create(user: user, post: post, text: 'comment 3', created_at: 7.days.ago)
      comment_four = Comment.create(user: user, post: post, text: 'comment 4', created_at: 5.days.ago)
      comment_five = Comment.create(user: user, post: post, text: 'comment 5', created_at: 3.days.ago)

      expect(post.five_most_recent_comments).to eq([comment_five, comment_four, comment_three, comment_two, comment_one])
    end
  end
end
